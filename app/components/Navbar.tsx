    ...
    <>
      <nav
        ...
        className={`fixed top-0 left-0 right-0 transition-all duration-500 border-b ${
          scrolled
            ...
            : 'bg-black py-6 border-transparent'
        }`}
      >
        <div className="max-w-[1400px] mx-auto flex items-center justify-between px-6 md:px-12">
          ...
          <motion.button
            ...
            className={`group flex items-center gap-3 text-2xl font-black uppercase tracking-tighter relative transition-colors duration-300 ${scrolled ? 'text-text' : 'text-white'}`}
          >
            <motion.span
              className="relative"
              whileHover={{ x: -2 }}
            ...
            >
              <motion.div
                ...
                whileHover={{ scale: [1, 1.5, 1], opacity: [0.5, 0, 0.5] }}
          ...
          <div className="hidden md:flex items-center gap-12">
            <div className="flex items-center gap-8">
              {navItems.map((item) => (
                <motion.button
                  ...
                  className={`group relative px-4 py-2 text-sm font-bold uppercase tracking-[0.15em] transition-all duration-300 ${scrolled ? 'text-text/70 hover:text-text' : 'text-white/70 hover:text-white'}`}
                  whileHover={{ y: -2 }}
                  whileTap={{ y: 0 }}
                  transition={{ type: "spring", stiffness: 400, damping: 17 }}
                >
            ...
            <motion.a
              ...
              className={`group relative px-6 py-3 rounded-full text-sm font-semibold uppercase tracking-[0.1em] transition-all duration-300 overflow-hidden border ${scrolled ? 'bg-brand-red/10 border-brand-red/30 text-brand-red hover:bg-brand-red hover:text-text' : 'bg-white/10 border-white/20 text-white hover:bg-white hover:text-black'}`}
          ...
          <button 
            ...
            style={{ zIndex: 110 }}
            className={`md:hidden relative w-12 h-12 flex flex-col items-center justify-center rounded-xl border transition-all ${scrolled ? 'bg-brand-blue/10 border-white/5' : 'bg-white/10 border-white/20'}`}
          >
            <div className="w-6 h-4 flex flex-col justify-between">
              <motion.span animate={{ rotate: menuOpen ? 45 : 0, y: menuOpen ? 7 : 0 }} className={`block h-[2px] w-full rounded-full origin-center ${scrolled ? 'bg-text' : 'bg-white'}`} />
              <motion.span animate={{ opacity: menuOpen ? 0 : 1 }} className={`block h-[2px] w-full rounded-full ${scrolled ? 'bg-text' : 'bg-white'}`} />
              <motion.span animate={{ rotate: menuOpen ? -45 : 0, y: menuOpen ? -7 : 0 }} className={`block h-[2px] w-full rounded-full origin-center ${scrolled ? 'bg-text' : 'bg-white'}`} />
            </div>
          </button>
        </div>
      ...
      <AnimatePresence>
        {menuOpen && (
          ...
          >
            {/* ... rest of your existing menu code remains completely untouched ... */}
            ...
            <div className="flex-1 flex flex-col items-center justify-center z-10 px-6">
              ...
              >
                <p className="text-[10px] uppercase tracking-[0.5em] text-brand-red mb-4">// NAVIGATION</p>
                <h2 className="text-2xl font-black uppercase tracking-tight text-text">Explore the studio</h2>
              </motion.div>
              ...
              >
                <p className="text-muted text-sm uppercase tracking-[0.18em] mb-6">Ready to collaborate?</p>
                <a
                  href="https://wa.me/2347081330590"
