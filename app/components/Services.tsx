    ...
    <section className="relative bg-[#080808] py-12 px-6 overflow-hidden">
      ...
      <div className="max-w-[1100px] mx-auto relative z-10">
        <div className="flex justify-between items-end mb-10 border-b border-white/5 pb-6">
          ...
          <span className="font-mono text-[10px] text-white/20 tracking-[0.3em] hidden sm:block">
            Ver. 2.0 // High_Speed
          </span>
        </div>
        ...
        <div className="flex flex-col">
          {SERVICES.map((service) => {
            ...
            return (
              <div key={service.id} className="border-b border-white/5 overflow-hidden will-change-transform">
                <button 
                ...
                >
                  <div className="flex items-center gap-6 md:gap-10">
                    <span className={`font-mono text-xs transition-colors duration-500 ${isOpen ? 'text-brand-red' : 'text-white/20'}`}>
                      {service.id}
                    </span>
                    <h3 className={`text-xl md:text-3xl font-black tracking-tight transition-all duration-500 ${isOpen ? 'text-white' : 'text-white/40 group-hover:text-white/70'}`}>
                      {service.title}
                    </h3>
                  </div>
        ...
        <div className="mt-10 flex justify-end">
          <Link href="https://wa.me/2347081330590" className="group flex items-center gap-3 bg-white text-black px-6 py-3 rounded-full hover:bg-brand-red hover:text-white transition-all duration-500 ease-[0.22,1,0.36,1]">
